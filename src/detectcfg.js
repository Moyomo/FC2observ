...
module.exports = {
	...
	search: () => {
		...
		if (result && result.game && fs.existsSync(result.game.path)) {
			...
			let configPath = path.join(result.game.path, "game", "core", "cfg", "gamestate_integration_fc2observ.cfg")

			if (fs.existsSync(configPath)) {
				let foundHeader = fs.readFileSync(configPath, "utf8").split("\n")[0]
				let ownHeader = fs.readFileSync(path.join(__dirname, "gamestate_integration_fc2observ.cfg"), "utf8").split("\n")[0]
	...
	install: (target) => {
		let template = path.join(__dirname, "gamestate_integration_fc2observ.cfg")
		let dest = path.join(target, "game", "core", "cfg", "gamestate_integration_fc2observ.cfg")
